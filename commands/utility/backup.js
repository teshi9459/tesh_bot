...
module.exports = {
  ...
  async execute(interaction) {
    if (
      ...
      !interaction.user.id === "652959577293324288"
    ) {
      interaction.reply({
        content: `du bist nicht befugt das zu machen`,
        ephemeral: true,
      });
      return;
    }
    ...
    output.on("close", function () {
      ...
      if (archive.pointer() >= 8 * 1000 * 1000) {
        ...
        interaction.editReply({
          content:
            "``` Backup wurde erstellt ✓ ```\nbitte speicher die Datei ab\ndie Datei wird in 3 min gelöscht",
          components: [row],
        });
        setTimeout(function () {
        ...
        interaction.editReply({
          content:
            "``` Backup wurde erstellt ✓ ```\nbitte speicher die Datei ab\ndiese Nachricht wird in 1 min gelöscht",
          files: ["./backup.zip"],
        });
        setTimeout(function () {
          interaction.deleteReply();
