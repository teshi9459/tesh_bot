  ...
  async execute(interaction) {
    ...
    await interaction.channel.messages.fetch().then((messages) => {
      for (let i = 0; i < filters.length; i++) {
        for (
          let j = 0;
          j < messages.filter((msg) => msg.content.startsWith(filters[i])).size;
          j++
        ) {
          delets.push(
            messages.filter((msg) => msg.content.startsWith(filters[i])).at(j)
              .id
          );
        ...
        for (
          let j = 0;
          j < messages.filter((msg) => msg.content.endsWith(filters[i])).size;
          j++
        ) {
          delets.push(
            messages.filter((msg) => msg.content.endsWith(filters[i])).at(j).id
          );
          delets = delets.filter(function (ele, pos) {
            return delets.indexOf(ele) == pos;
          });
    ...
    for (let i = 0; i < delets.length; i++) {
      ...
      await interaction.editReply(
        `\` ${del} \` von \` ${all} \` Nachrichten gelöscht!`
      );
      count++;
    }
    count -= skip;
    ...
    await interaction.editReply(
      `\` ${count} \` Nachrichten von gesamt \` ${all} \` gelöscht, welche ${filters} enthielten. Übersprungen: \` ${skip}\``
    );
  },
};
