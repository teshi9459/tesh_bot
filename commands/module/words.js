  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName("max")
        .setDescription(
          "Anzahl an Wörten die mindestens erreicht werden müssen"
        )
        .addIntegerOption((option) =>
          option
            .setName("amount")
            .setDescription("Zahl min 1")
            .setRequired(true)
        )
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName("min")
        .setDescription("Anzahl an Wörtern ab denen das Modul startet")
        .addIntegerOption((option) =>
          option
            .setName("amount")
            .setDescription("Zahl min 0")
            .setRequired(true)
        )
    )
    .addSubcommand((subcommand) =>
      subcommand.setName("status").setDescription("gibt die Moduldaten zurück")
    )
    .addSubcommand((subcommand) =>
      subcommand.setName("setup").setDescription("erstellt das words Modul")
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName("power")
