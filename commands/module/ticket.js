...
module.exports = {
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option
            .setName("typ")
            .setDescription("typ des pannels")
            .setRequired(true)
            .addChoice("Support", "support")
            .addChoice("Steckbrief", "char")
            .addChoice("Bewerbung", "team")
        )
        .addStringOption((option) =>
          option
            .setName("info")
            .setDescription("info auf dem Pannel")
            .setRequired(true)
        )
        .addChannelOption((option) =>
          option
            .setName("kategorie")
            .setDescription("Kategorie in die neue Tickets verschoben werden")
            .setRequired(true)
        )
    )
    .addSubcommand((subcommand) =>
      subcommand.setName("setup").setDescription("erstellt das Ticket System")
    )
    .addSubcommand((subcommand) =>
      subcommand.setName("delete").setDescription("löscht Pannel")
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName("rename")
  ...
  async execute(interaction) {
    ...
    switch (interaction.options.getSubcommand()) {
      ...
      case "rename":
        const ticket = t.getJ(
          `./DB/${interaction.guildId}/tickets/${interaction.channel.id}.json`
        );
        let typ = "";
        let icon = "";
        switch (ticket.typ) {
